...
for task in "${tasks[@]}"; do
    ...
    uv run python eval.py --config configs/${task}.yaml --model_name_or_path $model_name --endpoint_url "$ip" --use_vllm_serving --output_dir "output/$bname" --seq_len_filter $seq_len_filter &
done
wait
